...
class AnnouncementInput(BaseModal, title="공지 작성하기"):
    ...
    description = discord.ui.TextInput(
        label="공지 본문", style=discord.TextStyle.long, required=True, max_length=1024
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name=f"🔹 {self.a_title.value} - `1초 전`", value=self.description.value
        )
        ...
        if view.value:
            ...
            await db.user.update_many({}, {"$set": {"alerts.announcements": False}})
...
class NoticeInput(BaseModal, title="알림 보내기"):
    msg = discord.ui.TextInput(
        label="알림 내용", style=discord.TextStyle.long, required=True, max_length=1024
    )
...
class SendNotice(BaseView):
    ...
    async def msg_input(
        ...
        await interaction.response.send_modal(
            NoticeInput(ctx=self.ctx, target=self.target)
        )
        button.disabled = True
        await self.message.edit(view=self)
        self.value = True
...
class DataInput(BaseModal, title="데이터 수정하기"):
    ...
    data_value = discord.ui.TextInput(
        label="수정할 값", style=discord.TextStyle.long, required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name=f"수정할 데이터: {self.data_path.value}",
        ...
        if view.value:
            await self.colection.update_one(
                {"_id": self.target}, {"$set": {self.data_path.value: final_data}}
            )
        self.stop()
...
class ModifyData(BaseView):
    ...
    async def modify_user(
        ...
        if isinstance(self.target, str) and re.match(
            r"<@!?(\d+)>$", self.target
        ...
        if self.target in [g.id for g in self.ctx.bot.guilds]:
            ...
            return self.stop()
        await interaction.response.send_modal(
            DataInput(ctx=self.ctx, target=self.target, collection=collection)
        )
        self.value = True
        self.stop()

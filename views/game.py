...
class HostGuildGame(BaseView):
    ...
    async def join_game(
        ...
        if interaction.user in self.game.players:
            return await interaction.response.send_message(
                "<:denyed:730319627623596032> 이미 게임에 참가했습니다.", ephemeral=True
            )
        if await read(interaction.user, "banned.isbanned"):
            return await interaction.response.send_message(
                "<:denyed:730319627623596032> 차단된 유저는 게임에 참가할 수 없습니다.", ephemeral=True
            )
        self.game.players.append(interaction.user)
        await self.ctx.send(
            f"<:plus:988352187522506782> **{interaction.user}** 님이 참가했습니다."
        )
        ...
        self.message = await self.game.update_embed(
            self.game.hosting_embed(), view=self
        )

    @discord.ui.button(
        label="나가기", style=discord.ButtonStyle.red, emoji="<:leave:988350673223548949>"
    )
    async def leave_game(
        ...
        if interaction.user not in self.game.players:
            return await interaction.response.send_message(
                "<:denyed:730319627623596032> 게임에 참가하지 않았습니다.", ephemeral=True
            )
        self.game.players.remove(interaction.user)
        self.game.last_host = self.game.host
        await self.ctx.send(
            f"<:minus:988352203200794634> **{interaction.user}**님이 나갔습니다."
        )
    ...
    @discord.ui.button(
        ...
        emoji="<:start:988350697873477683>",
    )
    async def start_game(
        ...
        if interaction.user != self.game.host:
            return await interaction.response.send_message(
                "<:denyed:730319627623596032> 호스트만 게임을 시작할 수 있습니다.", ephemeral=True
            )
        if len(self.game.players) < 2:
            return await interaction.response.send_message(
                "<:denyed:730319627623596032> 플레이어 수가 부족하여 게임을 시작할 수 없습니다.",
                ephemeral=True,
            )

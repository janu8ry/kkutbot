...
class SoloGame(GameBase):
    ...
    async def send_info_embed(
        self,
        _msg: Union[discord.Message, KkutbotContext],
        desc: str = "10초 안에 단어를 이어주세요!",
    ) -> discord.Message:
        embed = discord.Embed(
        ...
        embed.add_field(
            name="🔹 단어",
            value=f"```yaml\n{self.bot_word} ({' / '.join(get_transition(self.bot_word))})```",
            inline=False,
        )
        embed.add_field(
            name="🔹 남은 시간",
            value=f"<t:{round((15 if self.kkd else 10) + self.begin_time)}:R>",
            inline=False,
        )
    ...
    async def game_end(self, result: Literal["승리", "패배", "포기"]):
        ...
        if result in ("패배", "포기"):
            ...
            if possibles:
                ...
                embed.add_field(
                    name="🔹 가능했던 단어",
                    value=f"`{'`, `'.join(possibles[:3])}` {'등...' if len(possibles) > 1 else ''}",
                    inline=False,
                )
            else:
                embed.add_field(
                    name="🔹 가능했던 단어",
                    value=f"`{self.bot_word}`은(는) 한방단어였습니다...",
                    inline=False,
                )
...
class MultiGame(GameBase):
    ...
    def hosting_embed(self) -> discord.Embed:
        ...
        embed.add_field(
            name=f"🔸 플레이어 ({len(self.players)}/5)",
    ...
    def game_embed(self) -> discord.Embed:
        ...
        embed.add_field(
            name="🔹 플레이어",
            value=f"`{'`, `'.join([_x.name for _x in self.players if _x not in self.final_score])}`",
            inline=False,
        )
        embed.set_footer(text="'ㄲ도움'을 입력하여 규칙을 확인할 수 있습니다.")
        if self.final_score:
            embed.add_field(
                name="🔻 탈락자",
                value=f"`{'`, `'.join([_x.name for _x in self.final_score])}`",
                inline=False,
            )
        return embed
        ...
        for n, kv in enumerate(rank):
            if n < len(rank) - 1:
                ...
                await write(
                    ...
                    await get_winrate(kv[0], "guild_multi"),
                )
        embed = discord.Embed(
            title="📔 게임 종료!",
            description="\n".join(desc),
    ...
    async def send_info_embed(self, desc: str = "10초 안에 단어를 이어주세요!") -> discord.Message:
        ...
        return await self.msg.channel.send(
            f"{self.now_player.mention}님, {desc}",
            embed=embed,
            delete_after=10 - (time.time() - self.begin_time),
        )
    ...
    async def game(self, ctx: KkutbotContext, mode: app_commands.Choice[int] = None):
        ...
        **4. 게임모드**
        ...
        if mode is None:
            embed = discord.Embed(
                title="📔 끝말잇기",
                description="🔸 끝말잇기 게임의 모드를 선택해 주세요.",
                color=config("colors.general"),
            )
            ...
            while True:
                try:
                    msg = await self.bot.wait_for(
                        "message",
                        check=check,
                        timeout=15.0 - (time.time() - game.begin_time),
                    )
                    user_word = msg.content
                except asyncio.TimeoutError:
                    await game.game_end("패배")

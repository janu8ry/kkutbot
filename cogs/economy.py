...
class Economy(commands.Cog, name="경제"):
    ...
    async def get_start_point(self, ctx: commands.Context):
        ...
        if await self.bot.if_koreanbots_voted(ctx.author):
            if not read(ctx.author, 'start_point'):
                ...
                await ctx.send("+300 <:points:715547592578170880> 를 받았습니다!")
                write(ctx.author, 'start_point', True)
            else:
                await ctx.send("<:denyed:730319627623596032> 이미 지원금을 받았습니다.")
        else:
            embed = discord.Embed(
                description="<:denyed:730319627623596032> "
    ...
    @commands.cooldown(rate=1, per=3, type=commands.BucketType.user)
    async def quest(self, ctx: commands.Context):  # noqa
        """매일 퀘스트를 클리어하고 보상을 획득합니다."""
        embed = discord.Embed(title="데일리 퀘스트", color=config('colors.help'))  # noqa

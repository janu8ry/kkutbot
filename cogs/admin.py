...
class Admin(commands.Cog, name="관리자"):
    ...
    async def ban_user(
        self,
        ctx: KkutbotContext,
        user: discord.Member = commands.parameter(converter=KkutbotUserConverter),
        days: float = 1.0,
        *,
        reason: str = "없음",
    ):
    ...
    async def blocked_list(self, ctx: KkutbotContext):
        ...
        if not banned_users:
            ...
            embed = discord.Embed(title="차단 유저 목록", color=config("colors.help"))
    ...
    async def move_unused_users(
        self,
        ctx: KkutbotContext,
        days: int = 7,
        command_usage: int = 10,
        delete_data: str = "n",
    ):
        """미사용 유저들을 정리합니다."""
        cleaned = 0

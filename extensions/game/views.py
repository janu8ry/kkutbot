...
class HostGuildGame(BaseView):
    ...
    async def join_game(self, interaction: discord.Interaction, _button: discord.ui.Button):
        if interaction.user in self.game.players:
            return await interaction.response.send_message(fmt("{denied} 이미 게임에 참가했습니다."), ephemeral=True)
    ...
    async def leave_game(self, interaction: discord.Interaction, _button: discord.ui.Button):
        if interaction.user not in self.game.players:
            return await interaction.response.send_message(fmt("{denied} 게임에 참가하지 않았습니다."), ephemeral=True)
    ...
    async def start_game(self, interaction: discord.Interaction, _button: discord.ui.Button):
        if interaction.user != self.game.host:
            return await interaction.response.send_message(fmt("{denied} 호스트만 게임을 시작할 수 있습니다."), ephemeral=True)
        if len(self.game.players) < 2:
            return await interaction.response.send_message(fmt("{denied} 플레이어 수가 부족하여 게임을 시작할 수 없습니다."), ephemeral=True)
